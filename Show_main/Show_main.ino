...
int vib_max=180; //change to something higher***************************************************
int brightness_max=254; //change to somethign higher*********************************************
int Sound_max=30;//change to somethign higher****************************************************
...
void morningVibrate()   //这个就是一个普通function
{
  ...
  /*
  if((wakeup_start_state()>=1)&&(motor_on==0)){
    if(((millis() - motor_lastUpdate) > morning_updateInterval)&&(motor_increase_rate<=254/motor_increase_unit))  {
      motor_lastUpdate = millis();
      motor_increase_rate+=1;
      
      currentmotor=motor_increase_unit*motor_increase_rate;
      Serial.print("motor speed: ");
      Serial.println(currentmotor);
      if (currentmotor>vib_max) {
        Serial.println("too strong++++++++++++++++++++++++++++++++++++++");
        currentmotor=vib_max;
        motor_on=1;
      }
      //analogWrite(motorPin, currentmotor);//just changed+++++++++++++++++++++++++++++
      analogWrite(motorPin, map(currentmotor, 0, 254, 34, max_motor));
      //delay(10);
    }
  }
  //after motor speed up to certain speed, this if statement maintain it at that speed
  if((wakeup_start_state()>=1)&&(motor_on==1)){
    Serial.println("motor on");
    analogWrite(motorPin, map(vib_max, 0, 254, 34, max_motor));
    motor_increase_rate=1;
  }
...
void checktime(){
 ...
 Serial.print(hour);
 Serial.print("h and ");
 Serial.print(minu%60);
 Serial.print("min and ");
 Serial.print(sec%3600);
 Serial.println("s of sleep");
 myFile.print(hour);
 myFile.print("h and ");
 myFile.print(minu%60);
 myFile.print("min and ");
 myFile.print(sec%3600);
 myFile.println("s of sleep");
 myFile.close();
 }
 }*/
void checktesttxt(){
...
void setup()
{
  ...
  Serial.println("initialization done.");
   checktesttxt();
   checksettingtxt();
  sleeptime=10000;//need to change before final************************************************************
}
